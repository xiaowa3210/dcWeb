...
function LoadBootstrapValidatorScript(callback){
	if (!$.fn.bootstrapValidator){
		$.getScript('static/newback/plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
$(document).ready(function () {
	...
	if (ajax_url.length < 1) {
		ajax_url = 'static/newback/ajax/dashboard.html';
	}
	...
	$('#screen_unlock').on('mouseover', function(){
		...
		var button = $('<div class="text-center"><a href="newindex.html" class="btn btn-primary">Unlock</a></div>');
