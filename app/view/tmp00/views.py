...
from app.model.models import db, User, Document
from app.view.tmp00 import tmp00
from app.view.exts import validate_login_register, validate_change_password
...
@tmp00.route('/tmp00/register/', methods=['GET', 'POST'])
def register():
    if request.method == 'GET':
        ...
        if '成功' in message:
            ...
            db.session.commit()
            return redirect(url_for('tmp00.login'))
        else:
            return render_template('tmp00/register.html')


@tmp00.route('/tmp00/login/', methods=['GET', 'POST'])
def login():
    if request.method == 'GET':
        ...
        if '成功' in message:
            ...
            return redirect(url_for('tmp00.home'))
        else:
            flash(message)
            return render_template('tmp00/login.html')


@tmp00.route('/tmp00/logout/')
def logout():
    session.clear()
    return redirect(url_for('tmp00.home'))

@tmp00.route('/tmp00/user/')
def user_center():
    return render_template('tmp00/user.html')


@tmp00.route('/tmp00/user/security/', methods=['GET', 'POST'])
def security():
    if request.method == 'GET':
...
@tmp00.route('/tmp00/user/user_news/', methods=['GET', 'POST'])
def user_news():
    if request.method == 'GET':
...
@tmp00.route('/tmp00/new_details/<document_id>/')
